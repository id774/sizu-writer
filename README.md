...
| `REQUIRE_SAME_ORIGIN` | `on` | Whether `POST /generate` must carry a browser `Origin` whose authority (host and port) matches the request's `Host`. A missing, malformed or foreign `Origin` is refused with status 400 before generation. Accepted booleans are `1`/`0`, `true`/`false`, `yes`/`no` and `on`/`off`, case-insensitively; any other value is refused. Turn it off only for a controlled non-browser client that intentionally posts to `/generate`. |
...
`POST /generate` is not meant as a public REST API: it is the form target of this application's own screens, and by default it refuses a request whose browser `Origin` does not belong to this same site (`REQUIRE_SAME_ORIGIN`, on by default). A request with no `Origin` header, a malformed one, or one naming a foreign host or port is refused with status 400 before the memo is even read, so a cross-site form on another page cannot spend a generation request or its budget. The check compares the `Origin` header's authority against the request's `Host`; it does not compare schemes, because Apache terminates TLS in front of gunicorn and `ProxyPreserveHost On` (already set in [deploy/sizu-writer.conf](deploy/sizu-writer.conf)) is what keeps the public `Host` visible to Flask. Submitting the same-host form in a browser, including the bundled `app.py` development server, is unaffected: the browser sends a matching `Origin` on its own. A rejected request never reaches `generate_draft()` or `regenerate_titles()`, its page does not echo back the submitted memo, Direction or body, and the rejection reason logged is one of `missing Origin`, `invalid Origin` or `foreign Origin` — never the raw header value. This check is a same-site boundary against cross-site form submission, not authentication; it is not a substitute for Basic authentication, IP restriction or a VPN in front of a published deployment (see [Deployment](#deployment)), and it can be disabled with `REQUIRE_SAME_ORIGIN=off` only for a controlled non-browser client that intentionally posts to `/generate` with a matching or absent `Origin`. `cli.py` never sends an HTTP request to this application and is unaffected by this setting.
...
| Module | Subject |
|---|---|
| `test_config.py` | environment driven settings, blank values, refusal of a malformed value, the accepted `LOG_LEVEL` values and its case-insensitive normalization, refusal of a legacy `OPENAI_*` variable, the base URL rules, the token kept out of `repr` and out of every message, the `MAX_POLICY_CHARS` default and its refusal of an invalid value, the static `Procfile` contract that it falls back to the documented default port of 8090 when `PORT` is unset, and the `REQUIRE_SAME_ORIGIN` default, its accepted boolean forms and its refusal of an unknown value |
